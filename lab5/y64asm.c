...
{
    symbol_t *cur_sym = symtab;
    while (cur_sym->next != NULL) {
        if (strcmp(cur_sym->next->name, name) == 0) {
            return cur_sym->next;
        }
...
int add_symbol(char *name)
{
    ...
    new_sym->name = malloc(strlen(name));
    strncpy(new_sym->name, name, strlen(name));
    /* add the new symbol_t to symbol table */
    cur_sym->next = new_sym;
    return 0;
}
...
int add_reloc(char *name, bin_t *bin)
{
    ...
    while (cur_reloc->next != NULL) {
        cur_reloc = cur_reloc->next;
    }
    /* create new reloc_t (don't forget to free it)*/
    reloc_t *new_reloc = malloc(sizeof(reloc_t));
    new_reloc->name = malloc(strlen(name));
    strncpy(new_reloc->name, name, strlen(name));
    new_reloc->y64bin = bin;
    /* add the new reloc_t to relocation table */
    cur_reloc->next = new_reloc;
    return 0;
}
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* set 'ptr' and 'regid' */
    const reg_t *reg_temp = find_register(*ptr);
    if (reg_temp == NULL) {
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    char *sym = *ptr;
    while (!IS_END(sym) && !IS_BLANK(sym) && *sym != ',') {
        sym++;
    }
    /* set 'ptr' and 'name' */
    if (sym == *ptr) return PARSE_ERR;
    *name = malloc(sym - *ptr + 1);
    strncpy(*name, *ptr, (sym - *ptr));
    *ptr = sym;
    return PARSE_SYMBOL;
}
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    char *next_ptr;
    *value = strtoull(*ptr, &next_ptr, 0);
    if (strcmp(*ptr, next_ptr) == 0) {
...
*           call SUM  #invoke SUM function */
    line->y64bin.addr = vmaddr; // execute firstly
    ...
    if (strchr(ptr, ':') != NULL) {
        char *_symbol;
        if (parse_label(&ptr, &_symbol) != PARSE_ERR) {
            if (add_symbol(_symbol) == 0) {
                symbol_t *cur_sym = symtab;
                while (cur_sym->next != NULL) {
                    cur_sym = cur_sym->next;
                }
                ...
                err_print("Dup symbol:%s", _symbol);
                line->type = TYPE_ERR;
                return line->type;
            }
            line->type = TYPE_INS;
    ...
    byte_t *bin_ptr = line->y64bin.codes;
    *bin_ptr = instr->code;
    bin_ptr++;
    /* update vmaddr */    
    vmaddr += instr->bytes;
    ...
    switch (HIGH(instr->code)) {
        ...
        {
            regid_t src, dest;
            if ((parse_reg(&ptr, &src) == PARSE_ERR) || (parse_delim(&ptr, ',') == PARSE_ERR) || (parse_reg(&ptr, &dest) == PARSE_ERR))
            {
                line->type = TYPE_ERR;
                return line->type;
            }
            *bin_ptr = (char)HPACK(src, dest);
            bin_ptr++;
            break;
        }
        case I_IRMOVQ:
        {
            ...
            long imm;
            char *name_p;
            parse_t status = parse_imm(&ptr, &name_p, &imm);
            if (status == PARSE_SYMBOL) {
                add_reloc(name_p, &line->y64bin);
            } else if (status == PARSE_ERR) {
                line->type = TYPE_ERR;
                return line->type;
            }
            if ((parse_delim(&ptr, ',') == PARSE_ERR) || (parse_reg(&ptr, &dest) == PARSE_ERR))
            {
                line->type = TYPE_ERR;
                return line->type;
            }
            *bin_ptr = (char)HPACK(REG_NONE, dest);
            bin_ptr++;
            long *lptr = (long *)bin_ptr;
            *lptr = imm;
            break;
        }
        case I_RMMOVQ:
        {
            regid_t src, mem_reg;
            long imm;
            if ((parse_reg(&ptr, &src) == PARSE_ERR) || (parse_delim(&ptr, ',') == PARSE_ERR) || (parse_mem(&ptr, &imm, &mem_reg) == PARSE_ERR))
            {
                line->type = TYPE_ERR;
                return line->type;
            }
            *bin_ptr = (char)HPACK(src, mem_reg);
            bin_ptr++;
            long *lptr = (long *)bin_ptr;
            *lptr = imm;
            break;
        ...
        case I_MRMOVQ:
        {
            regid_t mem_reg, dest;
            long imm;
            if ((parse_mem(&ptr, &imm, &mem_reg) == PARSE_ERR)|| (parse_delim(&ptr, ',') == PARSE_ERR) || (parse_reg(&ptr, &dest) == PARSE_ERR))
            {
                line->type = TYPE_ERR;
                return line->type;
            }
            *bin_ptr = (char)HPACK(dest, mem_reg);
            bin_ptr++;
            long *lptr = (long *)bin_ptr;
            *lptr = imm;
            break;
        ...
        case I_POPQ:
        {
            ...
            *bin_ptr = (char)HPACK(regA, REG_NONE);
            bin_ptr++;
            break;
        }
        case I_DIRECTIVE:
        {
            ...
            long imm;
            char *_symbol;
            parse_t status = parse_data(&ptr, &_symbol, &imm);
            if (status == PARSE_ERR) {
                line->type = TYPE_ERR;
                return line->type;
            } else if (status == PARSE_SYMBOL) {
                add_reloc(_symbol, &line->y64bin);
            }
            dtv_t ifun = LOW(instr->code);
            if (ifun == 0 && status == PARSE_DIGIT) {
                switch (instr->bytes) {
                    case 1:
                    {
                        *bin_ptr = imm;
                    ...
                    case 2:
                    {
                        __int16_t *sptr = (__int16_t *)bin_ptr;
                        *sptr = imm;
                        break;
                    }
                    case 4:
                    {
                        __int32_t *iptr = (__int32_t *)bin_ptr;
                        *iptr = imm;
                        break;
                    }
                    case 8:
