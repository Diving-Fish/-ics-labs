...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if (op == A_ADD) {
        ovf = (signA == signB) && (signA != signV);
    } else if (op == A_SUB) {
        ovf = (signA != signB) && (signB != signV);
    }
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
      {
        ...
        long_t adr = valB + imm;
        if (!set_long_val(sim->m, adr, valA)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        sim->pc = next_pc;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
      {
        ...
        long_t valB = get_reg_val(sim->r, regB);
        long_t adr = valB + imm;
        if (!get_long_val(sim->m, adr, &valA)) {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, adr);
            return STAT_ADR;
        }
      ...
      case I_CALL: /* 8:x imm */
      {
        ...
        if (!set_long_val(sim->m, stack, next_pc)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, stack);
            return STAT_ADR;
        }
        sim->pc = imm;
        break;
      ...
      case I_RET: /* 9:0 */
      {
        long_t stack = get_reg_val(sim->r, REG_RSP);
        long_t ret_adr;
        if (!get_long_val(sim->m, stack, &ret_adr)) {
            err_print("PC = 0x%lx, Invalid return address 0x%lx", sim->pc, stack);
            return STAT_ADR;
        }
        stack += 8;
        set_reg_val(sim->r, REG_RSP, stack);
      ...
      case I_PUSHQ: /* A:0 regA:F */
      {
        ...
        stack -= 8;
        set_reg_val(sim->r, REG_RSP, stack);
        if (!set_long_val(sim->m, stack, valA)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, stack);
            return STAT_ADR;
        }
        sim->pc = next_pc;
      ...
      case I_POPQ: /* B:0 regA:F */
      {
        long_t stack = get_reg_val(sim->r, REG_RSP);
        long_t valA;
        if (!get_long_val(sim->m, stack, &valA)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, stack);
            return STAT_ADR;
        }
        stack += 8;
